...
import {
  AZL_MAEI_ABOUT,
  AZL_MAEI_APPLICATIONS,
  AZL_MAEI_BENEFITS,
  AZL_MAEI_FAQ,
  AZL_MAEI_INTERNAL_LINKS,
  AZL_MAEI_INTRO,
  AZL_MAEI_PAGE_PATH,
  AZL_MAEI_PROCESS,
  AZL_MAEI_SIGNS,
...
const whatsappHref = `https://wa.me/${siteConfig.phone.replace(/\D/g, "")}?text=${encodeURIComponent(
  "مرحباً، أحتاج عزل مائي في جدة. أرجو تحديد موعد معاينة.",
)}`;
...
const galleryImages = [
  images.azlMaeiPageField,
  images.azlMaeiPageBitumenRoll,
  images.azlMaeiPageTankEpoxy,
] as const;
...
export function AzlMaeiJeddahPageBody() {
  const field = images.azlMaeiPageField;
  ...
  return (
    <main className="page-main pb-mobile-fab py-10 md:py-16">
      <nav className="mb-6 text-sm text-[#5a7588]" aria-label="مسار التصفح">
        ...
        <span className="mx-2">/</span>
        <span className="text-[#163d57]">عزل مائي بجدة</span>
      </nav>
      ...
      <section className="overflow-hidden rounded-3xl border border-[#e8edf0] bg-white shadow-[0_16px_32px_-24px_rgba(19,66,89,0.35)]">
        ...
        <div className="border-t border-[#e8edf0] bg-gradient-to-b from-[#f8fbfc] to-white p-6 text-right md:p-10">
          ...
          <p className="mt-3 max-w-3xl text-pretty text-base leading-8 text-[#4a6677] md:text-lg">
            حماية أسطح وخزانات وحمامات من تسربات الأمطار والرطوبة — معاينة ميدانية، مواد معتمدة،
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <div className="grid items-start gap-8 lg:grid-cols-2">
          <div>
            <h2 className={h2Class}>عن الخدمة — ما هو العزل المائي؟</h2>
            {AZL_MAEI_ABOUT.map((para) => (
            ...
            <p className={pClass}>
              ...
              عزل مائي تلائم نوع المبنى لا «حلّاً عاماً» ينسخ على كل سطح.
            </p>
          </div>
      ...
      <section className={cn(sectionClass, "mt-8")}>
        <h2 className={h2Class}>لماذا العزل المائي ضروري في جدة؟</h2>
        {AZL_MAEI_WHY.map((para) => (
          <p key={para.slice(0, 40)} className={pClass}>
            {renderBoldText(para)}
          </p>
        ))}
        <p className={pClass}>
          كثير من العملاء يصلون إلينا بعد تجربة «دهان سيليكون» أو طبقة رقيقة من مقاول عام — ثم
          يتفاجأون بعودة الرطوبة بعد أول أمطار. الفرق بين ذلك وبين{" "}
          <strong className="text-[#163d57]">عزل مائي بجدة</strong> احترافي يظهر في التحضير
          والمادة والاختبار، لا في لون الطبقة فقط.
        </p>
      </section>

      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <h2 className={h2Class}>أين نُطبّق العزل المائي؟</h2>
        <p className={pClass}>
          <strong className="text-[#163d57]">عزل مائي بجدة</strong> ليس حصراً على أسطح الفلل — نغطي
          كل المواضع التي يلمسها الماء:
        </p>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {AZL_MAEI_APPLICATIONS.map((item) => (
        ...
        <p className={pClass}>
          ...
          مطاعم — توقف نشاط يوم واحد بسبب تسرب سقف قد يكلف أكثر من العزل الوقائي. في{" "}
          <strong className="text-[#163d57]">الشقق</strong>، تسرب حمام غير معزول قد يصل لجار
          سفلي ويتحول لنزاع — العزل قبل السيراميك أرخص بكثير من إعادة تكسير كامل.
        </p>
      </section>

      <section className={cn(sectionClass, "mt-8")}>
        <h2 className={h2Class}>علامات تحتاج فيها إلى عزل مائي أو إعادة عزل</h2>
        <p className={pClass}>
          إن وُجدت <strong className="font-semibold text-[#163d57]">علامتان أو أكثر</strong>،
          المعاينة أولوية — خاصة قبل موسم الأمطار:
        </p>
        <ul className="mt-4 list-disc space-y-2 pr-6 leading-8 text-[#33586d] marker:text-[#1f7f8a]">
          {AZL_MAEI_SIGNS.map((sign) => (
        ...
        <figure className="relative mt-8 aspect-[1024/583] w-full max-w-2xl overflow-hidden rounded-2xl ms-auto">
          <Image
            src={images.azlMaeiPageMoisture.src}
            alt={images.azlMaeiPageMoisture.alt}
            title={images.azlMaeiPageMoisture.title}
            fill
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <h2 className={h2Class}>خطوات تنفيذ عزل مائي بجدة</h2>
        <ol className="mt-6 space-y-4">
          {AZL_MAEI_PROCESS.map((step, i) => (
      ...
      <section className={cn(sectionClass, "mt-8")}>
        ...
        <p className={pClass}>
          مناخ جدة الساحلي يفرض مواد تتحمل الرطوبة والملوحة — لا أنظمة «جافة» فقط:
        </p>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {AZL_MAEI_TECH.map((item) => (
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <h2 className={h2Class}>صور من أعمال العزل المائي</h2>
      ...
      <section className={cn(sectionClass, "mt-8")}>
        <h2 className={h2Class}>فوائد اختيار عزل مائي احترافي</h2>
        <ul className="mt-4 space-y-3 text-right">
          {AZL_MAEI_BENEFITS.map((b) => (
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        ...
        <p className={pClass}>
          ننفّذ <strong className="text-[#163d57]">عزل مائي بجدة</strong> في جميع أحياء المدينة —
          من شمال جدة حيث <strong className="text-[#163d57]">أبحر</strong> و
          <strong className="text-[#163d57]"> الشاطئ</strong> والفلل بأسطح واسعة، إلى{" "}
          <strong className="text-[#163d57]">الروضة</strong> و<strong className="text-[#163d57]">الصفا</strong>{" "}
          والعمائر الكثيفة، و<strong className="text-[#163d57]">الحمدانية</strong> و
          <strong className="text-[#163d57]">النسيم</strong> في الأحياء السكنية المتوسطة،
          و<strong className="text-[#163d57]">المنطقة الصناعية</strong> للمستودعات والهناجر.
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <h2 className={h2Class}>أسئلة شائعة عن العزل المائي في جدة</h2>
        <div className="mt-6 space-y-4">
          {AZL_MAEI_FAQ.map((item) => (
      ...
      <section className={cn(sectionClass, "mt-8")}>
        ...
        <p className={pClass}>
          <strong className="text-[#163d57]">عزل مائي بجدة</strong> استثمار في حماية مبناك من
          أمطار الساحل ورطوبته — لكنه ينجح فقط عندما يُبنى على معاينة صحيحة، إغلاق أي تسرب
          نشط، ومادة مناسبة لسطحك. لا تنتظر حتى يتساقط الجبس أو تتلف بضاعة في المستودع.
        </p>
        <p className={pClass}>
          اتصل اليوم أو راسلنا على واتساب — نُحدد موعد معاينة مجانية، نشرح خيارات البيتومين
          والسيكو بروف والإيبوكسي، ونُقدّم تقديراً واضحاً قبل أي التزام.
        </p>
      </section>
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        ...
        <ul className="mt-3 flex flex-wrap justify-end gap-2">
          {AZL_MAEI_INTERNAL_LINKS.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
      ...
      <section className="mt-8 rounded-3xl bg-gradient-to-l from-[#1f7f8a] to-[#163d57] p-8 text-right text-white md:p-10">
        <h2 className="text-2xl font-extrabold md:text-3xl">جاهز لعزل مائي في جدة؟</h2>
        <p className="mt-4 max-w-2xl text-base leading-8 text-white/95 md:text-lg">
          لا تنتظر موسم الأمطار. احجز معاينة — نحدد المادة والتكلفة بعد رؤية السطح أو الخزان
          أو الحمام.
        </p>
        <div className="mt-6 flex flex-row-reverse flex-wrap justify-end gap-3">
          <a
      ...
      <RelatedServicesSection currentPath={AZL_MAEI_PAGE_PATH} />
    </main>
  );
}
