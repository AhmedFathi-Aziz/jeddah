...
import { ArrowLeft, Layers, CheckCircle2, MapPin, Phone, ShieldCheck } from "lucide-react";
...
import {
  AZL_EPOXY_ABOUT,
  AZL_EPOXY_APPLICATIONS,
  AZL_EPOXY_BENEFITS,
  AZL_EPOXY_COMMON_MISTAKES,
  AZL_EPOXY_COMPARISON,
  AZL_EPOXY_COST_FACTORS,
  AZL_EPOXY_DISTRICTS,
  AZL_EPOXY_FAQ,
  AZL_EPOXY_INTERNAL_LINKS,
  AZL_EPOXY_INTRO,
  AZL_EPOXY_MAINTENANCE,
  AZL_EPOXY_PAGE_PATH,
  AZL_EPOXY_PROCESS,
  AZL_EPOXY_PROPERTY_TYPES,
  AZL_EPOXY_SIGNS,
  AZL_EPOXY_TECH,
  AZL_EPOXY_WHY,
  AZL_EPOXY_WHY_US,
...
const whatsappHref = `https://wa.me/${siteConfig.phone.replace(/\D/g, "")}?text=${encodeURIComponent(
  "مرحباً، أحتاج عزل إيبوكسي في جدة. أرجو تحديد موعد معاينة.",
)}`;
...
const galleryImages = [
  images.azlEpoxyPageFloor,
  images.azlEpoxyPageTankExternal,
] as const;
...
export function AzlEpoxyJeddahPageBody() {
  const field = images.azlEpoxyPageTankInterior;
  ...
  return (
    <main className="page-main pb-mobile-fab py-10 md:py-16">
      <nav className="mb-6 text-sm text-[#5a7588]" aria-label="مسار التصفح">
        ...
        <span className="mx-2">/</span>
        <span className="text-[#163d57]">عزل إيبوكسي بجدة</span>
      </nav>
      ...
      <section className="overflow-hidden rounded-3xl border border-[#e8edf0] bg-white shadow-[0_16px_32px_-24px_rgba(19,66,89,0.35)]">
        ...
        <div className="border-t border-[#e8edf0] bg-gradient-to-b from-[#f8fbfc] to-white p-6 text-right md:p-10">
          <p className="mb-2 inline-flex items-center gap-2 rounded-full bg-[#ecf8f8] px-3 py-1 text-sm font-semibold text-[#1f7f8a]">
            ...
            خزانات · أرضيات صناعية · بلمرة · جدة
          </p>
          ...
          <p className="mt-3 max-w-3xl text-pretty text-base leading-8 text-[#4a6677] md:text-lg">
            طلاء إيبوكسي للخزانات والأرضيات — تفريغ، تنظيف، طبقات معتمدة، اختبار عدم تسرب،
            وضمان على التنفيذ للفلل والعمائر والمستودعات.
          </p>
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <div className="grid items-start gap-8 lg:grid-cols-2">
          <div>
            <h2 className={h2Class}>عن الخدمة — ما هو عزل الإيبوكسي؟</h2>
            {AZL_EPOXY_ABOUT.map((para) => (
            ...
            <p className={pClass}>
              ...
              عزل إيبوكسي تلائم نوع المبنى لا «دهانة سطحية» تتقشر بعد موسم.
            </p>
          </div>
      ...
      <section className={cn(sectionClass, "mt-8")}>
        <h2 className={h2Class}>لماذا عزل الإيبوكسي ضروري في جدة؟</h2>
        {AZL_EPOXY_WHY.map((para) => (
          <p key={para.slice(0, 40)} className={pClass}>
            {renderBoldText(para)}
          </p>
        ))}
        <p className={pClass}>
          كثير من العملاء يصلون إلينا بعد تجربة «طلاء عادي» داخل الخزان من مقاول عام — ثم يتفاجأون
          برطوبة أو تلوث مياه. الفرق بين ذلك وبين{" "}
          <strong className="text-[#163d57]">عزل إيبوكسي بجدة</strong> احترافي يظهر في التفريغ
          والتجفيف وعدد الطبقات، لا في لون الطلاء فقط.
        </p>
      </section>

      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <h2 className={h2Class}>أين نُطبّق عزل الإيبوكسي؟</h2>
        <p className={pClass}>
          <strong className="text-[#163d57]">عزل إيبوكسي بجدة</strong> ليس حصراً على الخزانات —
          نغطي كل المواضع التي تحتاج طبقة بلمرية:
        </p>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {AZL_EPOXY_APPLICATIONS.map((item) => (
        ...
        <p className={pClass}>
          في <strong className="text-[#163d57]">المباني التجارية</strong> — مطابخ ومطاعم —
          إغلاق منطقة العزل يعني خسارة يوم عمل. في <strong className="text-[#163d57]">العمائر</strong>،
          خزان غير معزول قد يُضر الجميع — الإيبوكسي الداخلي أرخص من استبدال الخزان.
        </p>
      </section>

      <section className={cn(sectionClass, "mt-8")}>
        <h2 className={h2Class}>علامات تحتاج فيها إلى عزل إيبوكسي أو إعادة طلاء</h2>
        <p className={pClass}>
          إن وُجدت <strong className="font-semibold text-[#163d57]">علامتان أو أكثر</strong>،
          المعاينة أولوية — خاصة قبل موسم الأمطار أو تشغيل خزان جديد:
        </p>
        <ul className="mt-4 list-disc space-y-2 pr-6 leading-8 text-[#33586d] marker:text-[#1f7f8a]">
          {AZL_EPOXY_SIGNS.map((sign) => (
        ...
        <figure className="relative mt-8 aspect-[1024/583] w-full max-w-2xl overflow-hidden rounded-2xl ms-auto">
          <Image
            src={images.azlEpoxyPageMoisture.src}
            alt={images.azlEpoxyPageMoisture.alt}
            title={images.azlEpoxyPageMoisture.title}
            fill
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <h2 className={h2Class}>خطوات تنفيذ عزل إيبوكسي بجدة</h2>
        <ol className="mt-6 space-y-4">
          {AZL_EPOXY_PROCESS.map((step, i) => (
      ...
      <section className={cn(sectionClass, "mt-8")}>
        ...
        <p className={pClass}>
          رطوبة جدة الساحلية تؤثر على أوقات جفاف الإيبوكسي — نُخطّط الطبقات وفق درجة الحرارة والرطوبة
          ولا نُسرّع التصلب على حساب التصاق دائم:
        </p>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {AZL_EPOXY_TECH.map((item) => (
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <h2 className={h2Class}>مقارنة سريعة: متى يُستخدم الإيبوكسي ومتى البدائل؟</h2>
        <p className={pClass}>
          ليس كل مشكلة رطوبة تحتاج نفس الحل — نوضّح الخيار المناسب قبل التعاقد:
        </p>
        <div className="mt-6 grid gap-4 md:grid-cols-2">
          {AZL_EPOXY_COMPARISON.map((item) => (
            <article key={item.title} className="rounded-xl border border-[#e8edf0] bg-white p-5">
              <h3 className="font-bold text-[#163d57]">{item.title}</h3>
              <p className="mt-2 text-sm leading-8 text-[#4a6677] md:text-base">{renderBoldText(item.body)}</p>
            </article>
      ...
      <section className={cn(sectionClass, "mt-8")}>
        <h2 className={h2Class}>ما الذي يُحدّد تكلفة عزل الإيبوكسي في جدة؟</h2>
        <p className={pClass}>
          {renderBoldText(
            "عند طلب **سعر عزل إيبوكسي بجدة**، هذه العوامل تظهر في العرض بعد المعاينة — شفافية تمنع مفاجآت لاحقة:",
          )}
        </p>
        <ul className="mt-4 list-disc space-y-2 pr-6 leading-8 text-[#33586d] marker:text-[#1f7f8a]">
          {AZL_EPOXY_COST_FACTORS.map((item) => (
            <li key={item.slice(0, 40)}>{renderBoldText(item)}</li>
          ))}
        </ul>
      </section>

      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <h2 className={h2Class}>صيانة ما بعد عزل الإيبوكسي</h2>
        <p className={pClass}>
          الطبقة الجيدة تدوم أطول مع صيانة بسيطة — خاصة في **الفلل** و**المستودعات** ذات الاستخدام المكثف:
        </p>
        <ul className="mt-4 list-disc space-y-2 pr-6 leading-8 text-[#33586d] marker:text-[#1f7f8a]">
          {AZL_EPOXY_MAINTENANCE.map((item) => (
            <li key={item.slice(0, 40)}>{renderBoldText(item)}</li>
          ))}
        </ul>
      </section>
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <h2 className={h2Class}>صور من أعمال عزل الإيبوكسي</h2>
      ...
      <section className={cn(sectionClass, "mt-8")}>
        <h2 className={h2Class}>فوائد اختيار عزل إيبوكسي احترافي</h2>
        <ul className="mt-4 space-y-3 text-right">
          {AZL_EPOXY_BENEFITS.map((b) => (
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        ...
        <p className={pClass}>
          {AZL_EPOXY_DISTRICTS.map((para) => (
            <span key={para.slice(0, 50)} className="mt-4 block first:mt-0">
              {renderBoldText(para)}
            </span>
          ))}
      ...
      <section className={cn(sectionClass, "mt-8")}>
        <h2 className={h2Class}>أنواع العقارات التي ننفّذ لها عزل إيبوكسي في جدة</h2>
        <p className={pClass}>
          {renderBoldText("**عزل إيبوكسي بجدة** يختلف تفصيله حسب نوع المبنى — لا نطبّق نفس المخطط على كل مشروع:")}
        </p>
        <ul className="mt-4 list-disc space-y-2 pr-6 leading-8 text-[#33586d] marker:text-[#1f7f8a]">
          {AZL_EPOXY_PROPERTY_TYPES.map((item) => (
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        ...
        <p className={pClass}>
          تجنّب هذه الأخطاء يوفر إعادة تفريغ الخزان وتكلفة طلاء ثانية:
        </p>
        <ul className="mt-4 list-disc space-y-2 pr-6 leading-8 text-[#33586d] marker:text-[#1f7f8a]">
          {AZL_EPOXY_COMMON_MISTAKES.map((item) => (
            <li key={item.slice(0, 30)}>{renderBoldText(item)}</li>
          ))}
        </ul>
      </section>
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        <h2 className={h2Class}>أسئلة شائعة عن عزل الإيبوكسي في جدة</h2>
        <div className="mt-6 space-y-4">
          {AZL_EPOXY_FAQ.map((item) => (
      ...
      <section className={cn(sectionClass, "mt-8")}>
        ...
        <p className={pClass}>
          <strong className="text-[#163d57]">عزل إيبوكسي بجدة</strong> استثمار في سلامة مياهك
          وحماية مبناك — لكنه ينجح فقط عندما يُبنى على تفريغ وتجفيف حقيقيين، ومعالجة الشروخ،
          وطبقات بسماكة محددة. لا تنتظر حتى يتلوث الماء أو تتآكل أرضية المستودع.
        </p>
        <p className={pClass}>
          اتصل اليوم أو راسلنا على واتساب — نُحدد موعد معاينة مجانية، نشرح خيارات الإيبوكسي
          للخزانات والأرضيات، ونُقدّم تقديراً واضحاً قبل أي التزام.
        </p>
      </section>
      ...
      <section className={cn(sectionClass, "mt-8 bg-[#f7f9fa]")}>
        ...
        <ul className="mt-3 flex flex-wrap justify-end gap-2">
          {AZL_EPOXY_INTERNAL_LINKS.map((link) => (
            <li key={link.href}>
              <Link
                href={link.href}
      ...
      <section className="mt-8 rounded-3xl bg-gradient-to-l from-[#1f7f8a] to-[#163d57] p-8 text-right text-white md:p-10">
        <h2 className="text-2xl font-extrabold md:text-3xl">جاهز لعزل إيبوكسي في جدة؟</h2>
        <p className="mt-4 max-w-2xl text-base leading-8 text-white/95 md:text-lg">
          لا تُشغّل الخزان أو المستودع قبل العزل الصحيح. احجز معاينة — نحدد النظام والتكلفة
          بعد رؤية السطح ونتيجة أي فحص تسرب.
        </p>
        <div className="mt-6 flex flex-row-reverse flex-wrap justify-end gap-3">
          <a
            href={tel}
      ...
      <RelatedServicesSection currentPath={AZL_EPOXY_PAGE_PATH} />
    </main>
  );
}
